ReviewsLightboxGlobalModel=function(){this.applicationResources=undefined;
jQuery(this).bind("fetchApplicationResourcesSuccess",this,this._fetchApplicationResourcesSuccess);
jQuery(this).bind("fetchApplicationResourcesError",this,this._fetchApplicationResourcesError);
...
this._applicationView=jQuery(this._applicationViewTemplate);
this._templateStyle=templateStyle;
this.dataFetcher=new GalleryDataFetcher(this.applicationModel);
jQuery(this.dataFetcher).bind("dataFetchCallback",this,this._galleryDataFetchCallback);
};
ReviewGallery.prototype.render=function(){if(!this.viewRendered){this.viewRendered=true;
this._containerView=jQuery("body").find("#"+this._widgetName);
...
jQuery(this._immersiveViewController).bind("hideImmersiveViewClicked",this,this._hideImmersiveViewClicked);
};
...
jQuery(myself).trigger("hideImmersiveViewClicked");
...
var myself=this;
jQuery(window).bind("hashchange",function(){jQuery(myself).trigger("hideImmersiveViewClicked");
myself.unbindBrowserBack();
});
}};
ReviewGallery.prototype.unbindBrowserBack=function(){jQuery(window).unbind("hashchange");
...
this._containerView=jQuery("#"+name);
this._applicationView=jQuery(this._applicationViewTemplate);
this._containerView.append(this._applicationView);
...
this.dataFetcher=new GalleryDataFetcher(this.applicationModel);
jQuery(this.dataFetcher).bind("dataFetchCallback",this,this._galleryDataFetchCallback);
jQuery(this.dataFetcher).bind("dataFetchCompleteCallback",this,this._galleryDataFetchCompleteCallback);
var myself=this;
jQuery("#"+name).bind("jumpToImageAtIndex",function(event,index){new LogMetricsRequest({"ReviewsLightBox:DP:Widget:ThumnailClicked":1});
myself._jumpToImageAtIndex(index);
});
jQuery("#"+name).bind("hideImmersiveView",function(){new LogMetricsRequest({"ReviewsLightBox:DP:Widget:SeeAllClicked":1});
...
jQuery(this.thumbnailViewController).bind("thumbnailClicked",this,this._thumbnailClicked);
};
...
jQuery(this.immersiveViewController).bind("hideImmersiveViewClicked",this,this._hideImmersiveViewClicked);
};
...
function ReviewsLightboxEmbedded(name,templateStyle){ReviewsLightbox.call(this,name,templateStyle);
...
function CustomerProfileLightbox(name,templateStyle){ReviewsLightbox.call(this,name,templateStyle);
...
jQuery(this._view).bind("backButtonClicked",this,this._backButtonClicked);
jQuery(this._view).bind("nextButtonClicked",this,this._nextButtonClicked);
jQuery(this._view).bind("hideButtonClicked",this,this._hideClicked);
jQuery(this._view).bind("moveToImageAtOffset",this,this._moveToImageAtOffset);
jQuery(this._view).bind("castHelpfulVote",this,this._castHelpfulVote);
jQuery(this._view).bind("castNotHelpfulVote",this,this._castNotHelpfulVote);
jQuery(this).bind("castVoteSuccess",this,this._castVoteSuccess);
jQuery(this).bind("castVoteError",this,this._castVoteError);
...
myself.modal=modal.create(jQuery("<div></div>"),{"content":popoverTemplate});
...
this.viewDom=jQuery(this._globalModel.replaceStringPlaceholders(templateToRender,templateData));
...
var myself=this;
imageView.load(function(){jQuery(this).css({"width":"auto","height":"auto"});
var containerDom=myself.viewDom.find(".imageView");
var aspectRatioOfContainer=containerDom.width()/containerDom.height();
if(this.width/this.height>=aspectRatioOfContainer){jQuery(this).css({"width":"inherit","height":"auto"});
}else{jQuery(this).css({"width":"auto","height":"inherit"});
}});
};
...
var myself=this;
jQuery(backButtonContainer).bind("click",function(){jQuery(myself).trigger("backButtonClicked");
});
jQuery(nextButtonContainer).bind("click",function(){jQuery(myself).trigger("nextButtonClicked");
});
jQuery(hideButton).bind("click",function(){jQuery(myself).trigger("hideButtonClicked");
});
if(this._templateStyle==="DESKTOP"){var imageView=this.viewDom.find(".imageViewContainer");
jQuery(imageView).bind("mouseleave",this,this._mouseMovedOutOfImmersiveView);
jQuery(imageView).bind("mouseenter",this,this._mouseMovedIntoImmersiveView);
...
jQuery.each(images,function(index,image){var templateData={url:image.thumbnail};
var thumbnail=jQuery(myself._globalModel.replaceStringPlaceholders(myself._thumbnailTemplate,templateData));
if(image.shouldHighlight){thumbnail.addClass("imageHighlight");
thumbnail.attr("offset",thumbnail.index);
}else{jQuery(thumbnail).bind("click",function(){jQuery(myself).trigger("moveToImageAtOffset",image.index);
});
...
var imageInformationRenderedTemplate=jQuery(this._globalModel.replaceStringPlaceholders(this._globalModel.getTemplate(templateToRender),templateData));
...
jQuery(sectionHeader).bind("click",this,this._imageInformationHeaderClicked);
...
jQuery(helpfulVoteButton).bind("click",this,this._helpfulButtonClicked);
jQuery(notHelpfulVoteButton).bind("click",this,this._notHelpfulButtonClicked);
...
jQuery(informationPanelHeader).bind("click",this,this._imageInformationHeaderClicked);
imageInformationContainerDom.css({"margin-top":"-78px"});
}}imageInformationDom.html(imageInformationRenderedTemplate);
};
...
function CompositeThumbnailViewController(parentViewDom,applicationModel){this._viewContainerDom=parentViewDom;
...
jQuery.each(images,function(index,image){var index=myself.tiles.length;
var previewTile=new ThumbnailPreviewTile(index,image.smallImage,64);
jQuery(previewTile.viewDom).bind("click",function(){jQuery(myself).trigger("thumbnailClicked",index);
});
...
ThumbnailPreviewTile.prototype._renderTemplate=function(){this.viewDom=jQuery(this._thumbnailTemplate);
};
...
jQuery(this).bind("fetchReviewsDataSuccess",this,this._fetchReviewsDataSuccess);
jQuery(this).bind("fetchReviewsDataError",this,this._fetchReviewsDataError);
};
...
}jQuery(myself).trigger("dataFetchCallback",data);
...
}}}else{jQuery(myself).trigger("dataFetchCompleteCallback");
}};
...
jQuery.ajax({type:"POST",url:url,data:params,timeout:timeout,tryCount:0,retryLimit:retryLimit,success:function(data){if(data instanceof Array){data={"elements":data};
}jQuery(publisher).trigger(triggerName+"Success",data);
jQuery(publisher).trigger(triggerName+"Done",arguments);
},error:function(errorXHR,errorStatus,errorThrown){this.tryCount++;
if(this.tryCount<=this.retryLimit){jQuery.ajax(this);
return;
}jQuery(publisher).trigger(triggerName+"Error",{errorXHR:errorXHR,errorStatus:errorStatus,errorThrown:errorThrown});
jQuery(publisher).trigger(triggerName+"Done",arguments);
}});
};
...
ImmersiveView.prototype._immersiveViewForMobile=' <div class="immersiveView" style="width:100%;height:100%;position:relative;background-color: rgb(0, 0, 0);overflow:hidden;"> <div class="hideButton hideButton-mobile"> <span style="text-align:center; margin: auto;font-weight:bold;">{{ hideButtonText }}</span> </div> <div class="imageViewContainer" style="width:100%;float:left;height:100%;position:relative;"> <div class="imageView" style="position:absolute;top:0;left:0;width:100%;height:100%;"> <div style="position:relative;width:100%;height:100%;"> <img class="imageElement" src="" style="position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;max-width:100%;max-height:100%;"> </div> </div> <div class="backButtonContainer"> <div class="back-button back-button-mobile"> </div> </div> <div class="nextButtonContainer"> <div class="next-button next-button-mobile"> </div> </div> </div> <div class="imageInformationContainer" style="width:100%;height:400px;background-color:transparent;float:left;margin-top:-78px;"> <div class="imageInformation imageInformationMobile" style="width:100%;height:100%;position:relative;"> </div> </div>';
ThumbnailPreviewTile.prototype._thumbnailTemplate=' <div class="thumbnailPreviewTile"> </div> ';
if(typeof window.P==="object"){P.when("jQuery").execute(function($){jQuery=$;
if(!window.reviewsLightboxGlobalModel){var model=new ReviewsLightboxGlobalModel();
P.when("reviewsLightbox-globalModel").execute(function(){window.reviewsLightboxGlobalModel=model;
try{window.P.register("reviewsLightbox-js",function(){return{};
});
}catch(e){}});
}});
}